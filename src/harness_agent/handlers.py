...
from harness_agent.compaction import ContextCompactor, SessionArchiveWriter
...
from harness_agent.turn_runner import (
    AgentTurnRunner,
    ToolCallCommandRunner,
    TurnEventPublisher,
    TurnRequestBuilder,
    TurnSuperseded,
    TurnSupersessionGuard,
)
...
class AgentTurnHandler:
    def __init__(
        ...
        *,
        ...
        context_archive_runtime: UserRuntime | None = None,
        max_tokens_per_model: int = 128_000,
        compaction_reserved_tokens: int = 15_000,
        ...
        compactor = None
        if context_archive_runtime is not None:
            compactor = ContextCompactor(
                bus=bus,
                llm=llm,
                projection=projection,
                archive_writer=SessionArchiveWriter(context_archive_runtime),
        ...
        publisher = TurnEventPublisher(bus)
