...
GenerationCurrentPredicate = Callable[[str, int], Awaitable[bool]]
...
class AiogramTelegramAdapter:
    def __init__(
        ...
        *,
        token: str,
        bus: EventBus,
        is_current_generation: GenerationCurrentPredicate | None = None,
    ) -> None:
        self._bot = Bot(token=token)
        self._dispatcher = Dispatcher()
        self._router = Router()
        self._bus = bus
        self._is_current_generation = (
            _always_current_generation
            if is_current_generation is None
            else is_current_generation
        )
    ...
    def subscribe_outbound(self) -> None:
        self._bus.subscribe(AgentTurnRequested, self.handle_agent_turn_requested)
    ...
    async def send_assistant_text(self, event: AssistantTextProduced) -> None:
        if event.reply_target is None or event.reply_target.kind != "telegram":
    ...
    async def handle_assistant_text(self, event: AssistantTextProduced) -> HandlerResult:
        if event.reply_target is None or event.reply_target.kind != "telegram":
            return ()
    ...
    async def handle_agent_turn_requested(
        self,
...
async def _always_current_generation(
